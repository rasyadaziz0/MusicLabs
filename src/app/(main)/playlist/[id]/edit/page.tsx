...
export default function EditPlaylistPage() {
  ...
  // If user doesn't own the playlist or is not logged in
  useEffect(() => {
    if (playlist && user && playlist.user_id !== user.id) {
      router.push(`/playlist/${playlistId}`);
    }
  }, [playlist, user, playlistId, router]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    updatePlaylistMutation.mutate({ id: playlistId!, payload: form }, {
      onSuccess: () => {
        router.push(`/playlist/${playlistId}`);
      },
      onError: (error: Error) => {
        console.error('Update playlist gagal:', error);
        setErrorMessage(friendlyUpdatePlaylistError);
      },
    });
  };
  ...
  return (
    <div className="mx-auto max-w-4xl pt-10 px-6 pb-24">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-[28px] font-bold tracking-tight text-white">Edit Playlist</h1>
        {user && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={updatePlaylistMutation.isPending || !form.name.trim()}

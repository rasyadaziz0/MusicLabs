...
export default function NowPlaying({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const activeLine = lyricsScrollRef.current.querySelector(
  ...
  useEffect(() => {
    ...
    const activeLine = mobileLyricsScrollRef.current.querySelector(
      `[data-lyric-index="${activeIndex}"]`
    ) as HTMLElement | null;
  ...
  useEffect(() => {
    ...
    setOpenMenuUpward(spaceBelow < estimatedHeight && spaceAbove > spaceBelow);
  }, [isMoreMenuOpen]);
  ...
  const handleShareAction = async () => {
    ...
    const sharePayload = {
      ...
      text: `${currentTrack.artists.primary.map((artist) => artist.name).join(', ')} - ${currentTrack.name}`,
    ...
    try {
      ...
      if (typeof navigator !== 'undefined' && 'clipboard' in navigator) {
        ...
        if (clipboard?.writeText) {
          await clipboard.writeText(currentTrack.url);
        }
      }
      setIsMoreMenuOpen(false);
    } catch (error) {

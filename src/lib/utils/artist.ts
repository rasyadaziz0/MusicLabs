...
export function stripArtistIdPrefix(rawId: string): string {
  return rawId
    .replace(/^itunes-artist-/, '')
    .replace(/^artist-/, '');
}

export function isSearchBasedId(rawId: string): boolean {
  return rawId.startsWith('itunes-search-') || rawId.startsWith('search-');
}

export function isDummyArtist(artist: { name?: string; picture?: string; picture_xl?: string; nb_album?: number }, rawId: string): boolean {
  const strippedId = stripArtistIdPrefix(rawId);
  return (
 ...
 */
export function getArtistLink(artist: { id?: string; name?: string; title?: string }): string {
  const id = artist.id || '';
...
export function parse(
  ...
  if (!input) {
    return [createUnknownArtist(defaultIdPrefix, images)];
  }
...
export function createUnknownArtist(prefix: string, images: ImageQuality[]): Artist {
  return {
    id: `${prefix}-unknown`,
    name: 'Unknown Artist',
    role: 'primary',

import { LrcWord, LrcLine } from '@/types/utils/lrc';
...
function parseYRC(yrc: string): LrcLine[] {
  if (!yrc) return [];
...
function addInstrumentalPlaceholders(
  sourceLines: LrcLine[],
  type: 'yrc' | 'lrc' = 'lrc',
): LrcLine[] {
...
function estimateLineDurations(lines: LrcLine[]): LrcLine[] {
...
export class LrcHelper {
  static parseLRC = parseLRC;
  static parseYRC = parseYRC;
  static addInstrumentalPlaceholders = addInstrumentalPlaceholders;
  static estimateLineDurations = estimateLineDurations;
}

...
export class ColorExtractor {
  private static cache = new LRUColorCache();

  public static async extract(imageUrl: string | null | undefined): Promise<DominantColors> {
    if (!imageUrl) {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          let palette = buckets.map(b => this.averageBucket(b));
          palette = palette.map(p => this.boostSaturation(p, 1.3));

          palette.sort((a, b) => {
            const hA = this.getHue(a);
            const hB = this.getHue(b);
            const dH = Math.min(Math.abs(hA - hB), 360 - Math.abs(hA - hB));
            if (dH < 20) {
              return this.getLuma(b) - this.getLuma(a); 
            }
            return hB - hA; 
          });

          if (palette.length < 4) {
            const extra = this.getDefaultColors().colors.slice(palette.length);
            palette = [...palette, ...extra.map(c => this.hexToRgb(c))];
          }

          const primary = palette[0];
          const luma = this.getLuma(primary);

          const result: DominantColors = {
            colors: palette.slice(0, 4).map(c => this.rgbToHex(c)) as [string, string, string, string],
            luminance: luma,
          };
      ...
      img.onerror = () => {
        const fallback = this.getDefaultColors();
  ...
  public static getDefaultColors(): DominantColors {
    return {
  ...
  private static getPixels(canvas: HTMLCanvasElement): RGB[] {
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    if (!ctx) return [];
  ...
  private static medianCut(pixels: RGB[], depth: number): RGB[][] {
  ...
  private static averageBucket(bucket: RGB[]): RGB {
  ...
  private static boostSaturation(color: RGB, factor: number): RGB {
    let { r, g, b } = color;
    r /= 255; g /= 255; b /= 255;
  ...
  private static rgbToHex({ r, g, b }: RGB): string {
    return '#' + [r, g, b].map(x => {
  ...
  private static getLuma({ r, g, b }: RGB): number {
    return (0.299 * r + 0.587 * g + 0.114 * b) / 255;
  }

  private static getHue({ r, g, b }: RGB): number {
    r /= 255; g /= 255; b /= 255;
    const max = Math.max(r, g, b), min = Math.min(r, g, b);
    let h = 0;
    if (max === min) return 0;

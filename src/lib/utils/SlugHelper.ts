...
export class SlugHelper {
   ...
   */
  static stripTrackIdPrefix(trackId: string): string {
   ...
   */
  static restoreTrackId(cleanId: string): string {
    if (/^\d+$/.test(cleanId)) {
   ...
   */
  static buildArtistSlug(artistName: string): string {
    return SlugHelper.slugify(artistName);
  }
   ...
   * // → "/musik/adele/hello/dQw4w9WgXcQ"
   */
  static buildTrackPath(artistName: string, title: string, trackId: string): string {
    const artistSlug = SlugHelper.buildArtistSlug(artistName);
    const titleSlug = SlugHelper.slugify(title);
    const cleanId = SlugHelper.stripTrackIdPrefix(trackId);
